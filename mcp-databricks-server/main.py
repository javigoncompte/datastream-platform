...
async def execute_sql_query(sql: str) -> str:
    """
    Executes a given SQL query against the
        Databricks SQL warehouse and returns the formatted results.

    Use this tool when you need to run specific SQL queries,
        such as SELECT, SHOW, or other DQL statements.
    This is ideal for targeted data retrieval or
        for queries that are too complex for the structured description tools.
    ...
    try:
        ...
        if status == "failed":
            ...
            error_message = sdk_result.get(
                "error", "Unknown error during SQL execution."
            )
            details = sdk_result.get("details", "No additional details provided.")
            ...
            return (
                f"Received an unexpected status from query execution: {status}. "
                f"Result: {sdk_result}"
            )
...
async def describe_uc_table(
    full_table_name: str, include_lineage: Optional[bool] = False
) -> str:
    """
    ...
    Optionally, it can include comprehensive lineage information
    ...
    **Use Cases:**
    ...
    - Debugging: trace data flow issues by examining
            both table dependencies/processing code
    - Documentation: understand the complete data ecosystem around a table

    The lineage information allows LLMs and tools to subsequently fetch the actual
    notebook
    ...
    Args:
        full_table_name: The fully qualified three-part name of the table
                                                (e.g., `catalog.schema.table`).
        include_lineage: Set to True to fetch and include comprehensive lineage (tables,
                                                                                notebooks,
                                                                                jobs).
                         Defaults to False. May take longer to retrieve
                         but provides rich context for
    ...
    try:
        details_markdown = await asyncio.to_thread(
            ...
            include_lineage=include_lineage or False,
        )
        return details_markdown
    except ImportError as e:
        return (
            f"Error initializing Databricks SDK utilities: {str(e)}. "
            f"Please ensure DATABRICKS_HOST and DATABRICKS_TOKEN are set.)"
        )
    except Exception as e:
        return (
            f"Error getting detailed table description for "
            f"'{full_table_name}': {str(e)}"
        )
...
async def describe_uc_catalog(catalog_name: str) -> str:
    """
    Provides a summary of a specific Unity Catalog,
    listing all its schemas with their names and descriptions.

    Use this tool when you know the catalog name and
    need to discover the schemas within it.
    This is often a precursor to describing a specific schema or table.
    The output is formatted in Markdown.

    Args:
        catalog_name: The name of the Unity Catalog to describe
            (e.g., `prod`, `dev`, `system`).
    """
    try:
...
async def describe_uc_schema(
    catalog_name: str, schema_name: str, include_columns: Optional[bool] = False
) -> str:
    """
    ...
    Set `include_columns=True` to get column information, which is crucial
    for query construction but makes the output longer.
    If `include_columns=False`, only table names and descriptions are shown,
    useful for a quicker overview.
    The output is formatted in Markdown.

    Args:
        catalog_name: The name of the catalog containing the schema.
        schema_name: The name of the schema to describe.
        include_columns: If True, lists tables with their columns.
        Defaults to False for a briefer summary.
    """
    try:
        details_markdown = await asyncio.to_thread(
            get_uc_schema_details,
        ...
        return (
            f"Error getting detailed schema description for "
            f"'{catalog_name}.{schema_name}': {str(e)}"
        )
...
async def list_uc_catalogs() -> str:
    ...
    Use this tool as a starting point to discover available data sources when you don't
    know specific catalog names.
